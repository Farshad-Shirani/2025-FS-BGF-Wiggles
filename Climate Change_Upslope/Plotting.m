...
load('Results\sol_climateChange_upslope_moderate')
...
%%%%%%%%%%%%%%%%%%%
incrementSize = 3; % Size of increments in plotting curves. For example, if time samples in simulationParameters.times increase with steps pf size 2, and incrementSize is 10, then curves are plotted at every 2*10 = 20T
...
Q1_edgeIndex = dsearchn(x', 36); % N1(:,end)=0 at approximately x=36
Q2_edgeIndex = dsearchn(x', 31.3); % N2(:,end)=0 at approximately x=31.3
...
plot(x, modelParameters.Q_opt' + fluctuationAmplitude,'k', 'LineWidth', 1);
