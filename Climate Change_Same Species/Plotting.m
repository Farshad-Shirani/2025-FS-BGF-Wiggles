...
load('Results\sol_climateChange_sameSpecies_10_5.mat')
...
numSamples = size(N1,2);

%%%%%%%%%%%%%%%%%%%
incrementSize = floor(numSamples/100); % Size of increments in plotting curves. For example, if time samples in simulationParameters.times increase with steps pf size 2, and incrementSize is 10, then curves are plotted at every 2*10 = 20T
%%%%%%%%%%%%%%%%%%%
...
plot(x, N2(:,1)','Color', darkGreen, 'LineWidth', 2);
plot(x, N1(:,1)','Color', orange, 'LineWidth', 2);
...
Q1_edgeIndex = dsearchn(x', 51); % N1(:,end)=0 at approximately x=70
Q2_edgeIndex = dsearchn(x', 29); % N2(:,end)=0 at approximately x=40
...
plot(x, modelParameters.Q_opt','k--', 'LineWidth', 1);
plot(x, modelParameters.Q_opt' + 5,'k', 'LineWidth', 1);
